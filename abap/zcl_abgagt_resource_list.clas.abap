...
CLASS zcl_abgagt_resource_list IMPLEMENTATION.
  ...
  METHOD if_rest_resource~post.
    ...
    IF ls_request-package IS INITIAL.
      lv_json_resp = '{"success":"","command":"LIST","error":"Package is required"}'.
    ...
    DATA(lo_command) = lo_factory->get_command( zif_abgagt_command=>gc_list ).

    IF lo_command IS NOT BOUND.
      lv_json_resp = '{"success":"","command":"LIST","error":"LIST command not found"}'.
    ...
    DATA ls_params TYPE zcl_abgagt_command_list=>ty_list_params.
    ls_params-package = ls_request-package.
    ls_params-type = ls_request-type.
    ls_params-name = ls_request-name.
    ls_params-limit = ls_request-limit.
    ls_params-offset = ls_request-offset.
