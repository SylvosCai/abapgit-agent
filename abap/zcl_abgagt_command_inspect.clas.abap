  ...
  METHOD zif_abgagt_command~execute.
    ...
    LOOP AT ls_params-files INTO lv_file.
      ...
      IF lv_obj_type IS NOT INITIAL AND lv_obj_name IS NOT INITIAL.
        " Skip DDLS objects - they cannot be validated via Code Inspector
        IF lv_obj_type = 'DDLS'.
          CLEAR ls_error.
          ls_error-line = '0'.
          ls_error-column = '0'.
          ls_error-text = |DDLS { lv_obj_type } { lv_obj_name }: Validation requires ADT/Eclipse|.
    ...
    " Add DDLS messages to result
    IF ls_result-errors IS INITIAL.
      ls_result-errors = VALUE #(
        FOR msg IN ls_result-errors
        ( msg ) ).
    ENDIF.

    " Check if there are only DDLS objects
    IF lt_objects IS INITIAL AND ls_result-errors IS NOT INITIAL.
      ls_result-success = abap_false.
